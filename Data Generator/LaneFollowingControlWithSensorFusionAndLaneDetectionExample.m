...
% of the ego vehicle.
% * Lateral control - Keep the ego vehicle traveling along the centerline
...
% <docid:mpc_ug#mw_2025d6a5-c63c-4468-b4a2-e15164d4ccbd Adaptive Cruise
% Control with Sensor Fusion>. For an example of lateral control using a
% lane keeping assist (LKA) system with lane detection, see
% <docid:mpc_ug#mw_58bd8d15-b052-495d-afe9-77b7bf20fac4 Lane Keeping Assist
% with Lane Detection>. The ACC example assumes ideal lane detection, and
% the LKA example does not consider surrounding vehicles.
%
...
%% Define Scenario
% Before opening the model, you can optionally change the scenario that the
% model simulates. This scenario selection is controlled by a callback
% function, |helperLFSetUp|, which runs when the model opens.
% 
% By default, the model simulates a cut-in scenario on a curved road. To
% change the default scenario used, either edit the setup script by
% clicking the *Edit Setup Script* button in the model or by calling
% |helperLFSetup| with a new input scenario. For example, the following
% syntax is equivalent to specifying the default scenario.
%
...
% initializes the data used by the model by running the scenario function 
% and loading constants needed by the Simulink model, such as the vehicle 
% model parameters, controller design parameters, road scenario, and 
% surrounding cars.
%
...
% areas. It also shows the tracks maintained by the
...
% which is the closest track in front of the ego vehicle in its lane. The
% ideal lane markings are also shown along with the synthetically detected
% left and right lane boundaries (shown in red).
...
% * The *Steering angle* plot shows that the steering angle for ego vehicle
% is smooth.
%
...
% angle measures the yaw angle difference between the ego vehicle and the
% road. The ISO 8855 to SAE J670E block inside the subsystem converts the
...
% 
% The goals for the <docid:mpc_ref#bscj78x_1 MPC Controller> block are to:
%
...
% <<../mpcLFfig.png>>
% 
% The MPC controller is designed within the Path Following Control (PFC)
% System block based on the entered mask parameters, and the designed MPC
% Controller is an adaptive MPC which updates the vehicle model at run
% time. The lane following controller calculates the longitudinal
...
% Reader> block reads the actor poses data from the base workspace scenario
% variable. The block converts the actor poses from the world coordinates
% of the scenario into ego vehicle coordinates. The actor poses are
% streamed on a bus generated by the block. The Scenario Reader block also
% generates the ideal left and right lane boundaries based on the position
% of the vehicle with respect to the scenario used in |helperLFSetUp|.
%
% The <docid:driving_ref#mw_2fa86dfd-52db-4f59-8781-7d954ae3e8dc Vision
% Detection Generator> block takes the ideal lane boundaries from the
% Scenario Reader block. The detection generator models the field of view
% of a monocular camera and determines the heading angle, curvature,
% curvature derivative, and valid length of each road boundary, accounting
% for any other obstacles. The
% <docid:driving_ref#mw_535e4736-f89b-4dc7-a609-b8c323c4ce32 Driving Radar
% Data Generator> block generates clustered detections from the
% ground-truth data present in the field-of-view of the radar based on the
% radar cross-section defined in the scenario.
...
%
% # Select the scenario by changing the scenario name input to |helperLFSetUp|.
