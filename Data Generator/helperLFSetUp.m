...
assignin('base','max_steer',0.4);       % Maximum steering       (rad)
assignin('base','min_steer',-0.4);      % Minimum steering       (rad) 
assignin('base','PredictionHorizon',15); % Prediction horizon     
%assignin('base','PredictionHorizon',30); % Default prediction horizon     

%% Create driving scenario
% The scenario name is a MATLAB function created by the Driving Scenario Designer App. 
defaultScenarioFcnName = "S_shaped";
validScenarioFcnNames = {
    'Zigzag_edited',...                  % scenarioId = 1
    'L_shaped',...                       % scenarioId = 2
    'S_shaped',...                       % scenarioId = 3
    'Oval_edited_clock',...              % scenarioId = 4
    'Oval_edited_counter',...            % scenarioId = 5
    };

%scenarioStopTimes = [19.82 17.99 21.99 25.88 26.93 39.51 34.28 22.80 23.47];
%scenarioStopTimes = [19.82 17.99 21.99 25.88 26.93 39.51 34.28 60 23.47 60];
...
assignin('base','m',2100);      % Total mass of vehicle                          (kg)
% assignin('base','Iz',2875);     % Yaw moment of inertia of vehicle               (m*N*s^2)
assignin('base','Iz',3833);     % Yaw moment of inertia of vehicle               (m*N*s^2)
...
% Corner stiffness: of a tire is its ability to resist deformation in the shape
% of a tire while the vehicle corners.
%% Bus Creation
